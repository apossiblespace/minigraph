...
(deftest pan-viewport-test
  ...
  (testing "Pan with zoom affects canvas delta"
    (let [vp (m/viewport {:width 800 :height 600 :x 0 :y 0 :zoom 2.0})
          panned (c/pan-viewport vp 100 50)]
...
(deftest fit-viewport-test
  (testing "Fit viewport to nodes"
    (let [vp (m/viewport {:width 800 :height 600})
          n1 (m/node {:id "n1" :x 0 :y 0 :width 100 :height 100})
          n2 (m/node {:id "n2" :x 300 :y 300 :width 100 :height 100})
          fitted (c/fit-viewport vp [n1 n2])]
      ;; Should zoom to fit both nodes with padding
      ;; Content is 400x400, with padding=50 → 500x500 to fit
      ;; Viewport 800x600: zoom = min(800/500, 600/500) = min(1.6, 1.2) = 1.2
      (is (approx= (:zoom fitted) 1.2 0.01))  ; Zoomed to fit exactly
      (is (<= (:x fitted) 0))                  ; Pan includes first node
      (is (>= (:zoom fitted) 0.1))))           ; Within min zoom

  (testing "Fit viewport with no nodes resets view"
    (let [vp (m/viewport {:width 800 :height 600 :x 100 :y 200 :zoom 0.5})
...
(deftest node-at-point-test
  (testing "Finds node at point"
    (let [vp (m/viewport {:width 800 :height 600 :x 0 :y 0 :zoom 1.0})
          n1 (m/node {:id "n1" :x 0 :y 0 :width 100 :height 100})
          n2 (m/node {:id "n2" :x 200 :y 200 :width 100 :height 100})
  ...
  (testing "Returns topmost node when overlapping"
    (let [vp (m/viewport {:width 800 :height 600 :x 0 :y 0 :zoom 1.0})
          n1 (m/node {:id "n1" :x 0 :y 0 :width 100 :height 100})
          n2 (m/node {:id "n2" :x 50 :y 50 :width 100 :height 100})
...
(deftest nodes-in-rect-test
  (testing "Finds nodes in selection rectangle"
    (let [vp (m/viewport {:width 800 :height 600 :x 0 :y 0 :zoom 1.0})
          n1 (m/node {:id "n1" :x 0 :y 0 :width 100 :height 100})
          n2 (m/node {:id "n2" :x 200 :y 200 :width 100 :height 100})
          n3 (m/node {:id "n3" :x 500 :y 500 :width 100 :height 100})
...
(deftest select-all-test
  (testing "Select all nodes"
    (let [n1 (m/node {:id "n1" :x 0 :y 0 :width 10 :height 10})
          n2 (m/node {:id "n2" :x 0 :y 0 :width 10 :height 10})
...
(deftest apply-drag-to-node-test
  (testing "Apply drag to single node"
    (let [node (m/node {:id "n1" :x 100 :y 200 :width 50 :height 50})
...
(deftest apply-drag-to-nodes-test
  (testing "Apply drag to multiple selected nodes"
    (let [n1 (m/node {:id "n1" :x 0 :y 0 :width 10 :height 10})
          n2 (m/node {:id "n2" :x 100 :y 100 :width 10 :height 10})
          n3 (m/node {:id "n3" :x 200 :y 200 :width 10 :height 10})
