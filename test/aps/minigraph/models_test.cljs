...
(deftest make-node-test
  (testing "Creates valid node with required fields"
    (let [node (m/make-node {:id "node-1"
                             :x 100
                             :y 200
                             :width 120
                             :height 80})]
  ...
  (testing "Creates node with custom type and data"
    (let [node (m/make-node {:id "node-2"
                             :x 0
                             :y 0
                             :width 100
                             :height 100
  ...
  (testing "Throws on invalid dimensions"
    (is (thrown? js/Error
                 (m/make-node {:id "bad" :x 0 :y 0 :width 0 :height 100})))
    (is (thrown? js/Error
                 (m/make-node {:id "bad" :x 0 :y 0 :width 100 :height -10})))))

(deftest node?-test
  (testing "Validates correct nodes"
    (is (m/node? (m/make-node {:id "n1" :x 0 :y 0 :width 10 :height 10}))))
...
(deftest update-node-position-test
  (testing "Updates node position"
    (let [node (m/make-node {:id "n1" :x 10 :y 20 :width 50 :height 50})
...
(deftest make-edge-test
  ...
  (testing "Throws on self-loop"
    (is (thrown? js/Error
                 (m/make-edge {:id "e1" :source "n1" :target "n1"})))))

(deftest edge?-test
  (testing "Validates correct edges"
    (is (m/edge? (m/make-edge {:id "e1" :source "n1" :target "n2"}))))
...
(deftest make-viewport-test
  ...
  (testing "Creates viewport with custom values"
    (let [vp (m/make-viewport {:x 100 :y 200 :zoom 0.5 :width 1024 :height 768})]
      (is (= 100 (:x vp)))
      (is (= 200 (:y vp)))
      (is (= 0.5 (:zoom vp)))))

  (testing "Throws on invalid values"
    (is (thrown? js/Error (m/make-viewport {:width 0 :height 600})))
    (is (thrown? js/Error (m/make-viewport {:width 800 :height 600 :zoom 0})))))

(deftest viewport?-test
  (testing "Validates correct viewport"
    (is (m/viewport? (m/make-viewport {:width 800 :height 600}))))
...
(deftest make-graph-test
  ...
  (testing "Creates graph with nodes and edges"
    (let [nodes [(m/make-node {:id "n1" :x 0 :y 0 :width 10 :height 10})]
          edges [(m/make-edge {:id "e1" :source "n1" :target "n2"})]
          g (m/make-graph {:nodes nodes :edges edges})]
      (is (= 1 (count (:nodes g))))
      (is (= 1 (count (:edges g)))))))

(deftest graph?-test
  (testing "Validates correct graph"
    (is (m/graph? (m/make-graph {}))))
...
(deftest find-node-test
  (testing "Finds existing node"
    (let [node (m/make-node {:id "n1" :x 0 :y 0 :width 10 :height 10})
          g (m/make-graph {:nodes [node]})
...
(deftest find-edge-test
  (testing "Finds existing edge"
    (let [edge (m/make-edge {:id "e1" :source "n1" :target "n2"})
          g (m/make-graph {:edges [edge]})
...
(deftest add-node-test
  (testing "Adds node to graph"
    (let [g (m/make-graph {})
          node (m/make-node {:id "n1" :x 0 :y 0 :width 10 :height 10})
...
(deftest add-edge-test
  (testing "Adds edge to graph"
    (let [g (m/make-graph {})
          edge (m/make-edge {:id "e1" :source "n1" :target "n2"})
...
(deftest update-node-test
  ...
  (testing "Leaves other nodes unchanged"
    (let [n1 (m/make-node {:id "n1" :x 0 :y 0 :width 10 :height 10})
          n2 (m/make-node {:id "n2" :x 50 :y 50 :width 10 :height 10})
          g (m/make-graph {:nodes [n1 n2]})
          updated (m/update-node g "n1" m/update-node-position 100 200)]
...
(deftest remove-node-test
  ...
  (testing "Removes connected edges when removing node"
    (let [n1 (m/make-node {:id "n1" :x 0 :y 0 :width 10 :height 10})
          ...
          e2 (m/make-edge {:id "e2" :source "n2" :target "n1"})
          g (m/make-graph {:nodes [n1 n2] :edges [e1 e2]})
          updated (m/remove-node g "n1")]
      (is (= 0 (count (:edges updated)))))))

(deftest remove-edge-test
  (testing "Removes edge from graph"
    (let [e1 (m/make-edge {:id "e1" :source "n1" :target "n2"})
          e2 (m/make-edge {:id "e2" :source "n2" :target "n3"})
          g (m/make-graph {:edges [e1 e2]})
